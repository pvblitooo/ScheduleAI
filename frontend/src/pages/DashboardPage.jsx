...
const ActionButtons = () => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
    <Link 
      to="/actividades" 
      // --- Clases MODIFICADAS ---
      className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-6 py-5 rounded-xl hover:border-purple-500/50 text-center transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-purple-500/10 dark:hover:shadow-purple-500/20"
    >
      <div className="w-12 h-12 bg-gradient-to-br from-purple-500/10 to-blue-500/10 dark:from-purple-500/20 dark:to-blue-500/20 rounded-lg flex items-center justify-center mx-auto mb-3 group-hover:from-purple-500/20 dark:group-hover:from-purple-500/30 transition-all">
      ...
      <span className="text-purple-600 dark:text-purple-400 text-base font-semibold block group-hover:text-purple-500 dark:group-hover:text-purple-300 transition-colors">
...
const DashboardPage = () => {
  ...
  return (
    // --- MODIFICADO: Quitamos 'text-white' y ponemos color de texto base ---
    <div className="p-6 sm:p-8 space-y-6 text-slate-900 dark:text-white min-h-screen">
      
      {/* --- SALUDO (MODIFICADO) --- */}
      {!isLoading && userName && (
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">
            ¡Hola, <span className="bg-gradient-to-r from-purple-500 to-blue-500 dark:from-purple-400 dark:to-blue-400 bg-clip-text text-transparent">{userName}</span>!
          </h1>
          {activeSchedule && (
            <p className="text-base text-slate-500 dark:text-slate-400">
              Tu rutina activa es: <span className="font-semibold text-purple-600 dark:text-purple-400">{activeSchedule.name}</span>
            </p>
          )}
        </div>
      )}
      ...
      <div className="space-y-6">
        ...
        {/* --- MODIFICADO: SI NO hay rutina activa --- */}
        {!isLoading && !activeSchedule && (
          <div className="text-center p-12 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-2xl mx-auto shadow-xl">
            ...
            <h2 className="text-2xl font-bold bg-gradient-to-r from-purple-500 to-blue-500 dark:from-purple-400 dark:to-blue-400 bg-clip-text text-transparent mb-3">
        ...
        {!isLoading && activeSchedule && (
          <>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {/* --- MODIFICADO: Próximas Tareas --- */}
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-xl">
                <h2 className="bg-gradient-to-r from-purple-500 to-blue-500 dark:from-purple-400 dark:to-blue-400 bg-clip-text text-transparent font-bold text-xl mb-5">
                  Próximas Tareas
                </h2>
                {upcomingEvents.length > 0 ? (
                  <ul className="space-y-3 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
                    {upcomingEvents.map(event => (
                      <li 
                        ...
                        // --- MODIFICADO: Estilos de la tarjeta de tarea ---
                        className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 hover:border-purple-400 dark:hover:border-purple-500/30 transition-all duration-200"
                      >
                        <div 
                        ...
                        <div className="flex-grow min-w-0">
                          ...
                          <p className="font-semibold text-slate-800 dark:text-white text-sm mb-1 truncate">{event.title}</p>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-sm">¡Genial! No tienes tareas próximas.</p>
                  </div>
                )}
              </div>

              {/* --- MODIFICADO: Distribución Semanal --- */}
              <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-xl flex flex-col">
                <h2 className="bg-gradient-to-r from-purple-500 to-blue-500 dark:from-purple-400 dark:to-blue-400 bg-clip-text text-transparent font-bold text-xl mb-5">
                ...
                {weeklyCategoryDistribution.length > 0 ? (
                    ...
                    <ResponsiveContainer>
                      <PieChart>
                        <Pie 
                          ...
                          fill="#8884d8" 
                          dataKey="value" 
                          // --- ¡¡AQUÍ ESTÁ LA CORRECCIÓN PARA LOS WARNINGS!! ---
                          // Quitamos '...props' y solo pasamos lo necesario.
                          label={({ name, percent, x, y, textAnchor }) => {
                            const labelColor = theme === 'light' ? '#334155' : '#cbd5e1'; // slate-700 o slate-300
                        ...
                        <Tooltip 
                          // --- MODIFICADO: Estilos del Tooltip dinámicos ---
                          contentStyle={{ 
                  ...
                  <div className="flex-grow flex items-center justify-center">
                    {/* --- MODIFICADO: Color de texto --- */}
                    <p className="text-slate-500 dark:text-slate-400 text-sm">No hay datos para mostrar la distribución.</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {/* --- MODIFICADO: SECCIÓN DE ACCIONES --- */}
        {!isLoading && (
          <div className="text-center pt-8 mt-6 border-t border-slate-200 dark:border-slate-800">
            <h2 className="text-xl font-semibold mb-6 text-slate-700 dark:text-slate-300">¿Qué quieres hacer ahora?</h2>
            <ActionButtons />
          </div>
        )}
      </div>
