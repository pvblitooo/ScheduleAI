      ...
      <div className="w-full max-w-sm">
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl rounded-2xl px-8 pt-8 pb-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-slate-700 dark:text-slate-300 text-sm font-medium mb-1.5" htmlFor="login-email">
                Email
              </label>
              <input 
                id="login-email" 
            ...
            <div>
              <label className="block text-slate-700 dark:text-slate-300 text-sm font-medium mb-1.5" htmlFor="login-password">
                Contraseña
              </label>
              ...
              <div className="relative">
                <input 
                  ...
                  required 
                  placeholder="••••••••"
                  // 3. Padding a la derecha (pr-10) para hacer espacio al icono
            ...
            <div className="flex items-center pt-0.5">
              <input
                ...
                className="peer relative appearance-none w-4 h-4 border-2 border-slate-400 dark:border-slate-700 rounded bg-slate-100 dark:bg-slate-800 cursor-pointer transition-all duration-200 checked:bg-gradient-to-br checked:from-purple-500 checked:to-blue-500 checked:border-transparent hover:border-purple-500"
              />
              ...
              <label htmlFor="remember-me" className="ml-2.5 text-xs text-slate-500 dark:text-slate-400 cursor-pointer select-none hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
                Recordarme
              </label>
            </div>
          ...
          {error && (
            <div className="mt-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-600 dark:text-red-300 px-3 py-2.5 rounded-lg text-xs">
              {error}
            </div>
          )}
          ...
          <div className="mt-5 pt-5 border-t border-slate-200 dark:border-slate-800 text-center">
            <p className="text-slate-500 dark:text-slate-400 text-xs">
              ¿No tienes cuenta?{' '}
              <Link to="/register" className="text-purple-500 dark:text-purple-400 hover:text-purple-600 dark:hover:text-purple-300 font-medium transition-colors duration-200">
                Regístrate
              </Link>
            </p>
          </div>
